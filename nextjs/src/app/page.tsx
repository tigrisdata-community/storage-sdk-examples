...
export default function Home() {
  ...
  const fetchFiles = useCallback(async (page = 1, limit = 10, paginationMarker?: string) => {
    try {
      setLoading(true)
  ...
  const handleDelete = async (key: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const currentMarker = paginationStack.length > 0 ? paginationStack[paginationStack.length - 1] : undefined
        await fetchFiles(pagination.currentPage, pagination.itemsPerPage, currentMarker)
      } else {
  ...
  const handlePageChange = (page: number) => {
    if (page > pagination.currentPage) {
      ...
      const prevMarker = newStack.length > 0 ? newStack[newStack.length - 1] : undefined
